...
function openEmbeddedSetup() {
  try {
    window.open(embeddedSetupURL, '_blank')
  } catch (error) {
...
async function connectAccount() {
  ...
  try {
    const { email: connectedEmail } = await authApi.addAccount(normalizedToken)
    oauthToken.value = ''
    isOpen.value = false
    emit('account-added')
...
async function addRawCredential() {
  ...
  try {
    await authApi.addRaw(credential)
    rawCredential.value = ''
    isOpen.value = false
    emit('account-added')
